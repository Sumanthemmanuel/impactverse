...
import { ArrowLeft, MapPin, AlertTriangle, Bot, ShieldCheck, Clock, FileText, CheckCircle2 } from "lucide-react";
import { api } from "@/lib/api";
import { getUserRole } from "@/lib/auth";
import styles from "./page.module.css";

export default function ChallengeDetailClient({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.grid}>
        <div className={styles.mainCol}>
          {/* Problem Tracker Timeline */}
          <div className={styles.card}>
            <h2>Problem Tracker</h2>
            <div className={styles.trackerTimeline}>
              {['SUBMITTED', 'MATCHED', 'VALIDATED', 'ACTION TAKEN', 'RESOLVED'].map((step, idx) => {
                const statuses = ['SUBMITTED', 'MATCHED', 'VALIDATED', 'DEPLOYED', 'RESOLVED'];
                const currentIdx = statuses.indexOf(challenge.status);
                let stepClass = styles.stepPending;
                if (currentIdx >= idx) stepClass = styles.stepCompleted;
                if (currentIdx === idx) stepClass = styles.stepActive;
                
                return (
                  <div key={step} className={`${styles.trackerStep} ${stepClass}`}>
                    <div className={styles.stepCircle}>
                      {currentIdx >= idx ? <CheckCircle2 size={16} /> : idx + 1}
                    </div>
                    <div className={styles.stepLabel}>{step}</div>
                    {idx < 4 && <div className={styles.stepLine}></div>}
                  </div>
                );
              })}
            </div>
